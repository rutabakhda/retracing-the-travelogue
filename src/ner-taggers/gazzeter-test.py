...
for index,row in data.iterrows():
    ...
    one_gram_list = common_1gram + common_alternative_1gram
    two_gram_list = common_bigram + common_alternative_bigram
    tri_gram_list = common_trigram + common_alternative_trigram
    ...
    ogram = ', '.join([str(elem) for elem in one_gram_list])
    tgram = ' ,'.join([str(elem) for elem in two_gram_list])
    thgram = ' ,'.join([str(elem) for elem in tri_gram_list])

    one_gram.append(ogram)
    two_gram.append(tgram)
    three_gram.append(thgram)
    count = count + 1
    print(count)
...
data['One Gram'] = one_gram
data['Two Gram'] = two_gram
data['Three Gram'] = three_gram

writefile = datapath / 'data/hugh-murray/chapter1/chapter1-gazetter-test.csv'
